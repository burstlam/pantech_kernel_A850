...
#ifdef CONFIG_SKY_SND_DOCKING_CRADLE  //20120810 jhsong : docking usb switch gpio
static void docking_enable_gpio(int enable)
{
	struct pm_gpio param = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
	...
	struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)==SND_SOC_DAPM_POST_PMU)
	{
		if ((!strncmp(w->name, "Ext Spk Bottom Pos", 18)) || (!strncmp(w->name, "Ext Spk Bottom Neg", 18))){
			...
			{
				pr_err("[APQ8064] spkamp_event SPK ON fail...\n");
				return -EINVAL;
			}
	...
	else
	{
		if ((!strncmp(w->name, "Ext Spk Bottom Pos", 18)) || (!strncmp(w->name, "Ext Spk Bottom Neg", 18))){
			...
			{
				pr_err("[APQ8064] spkamp_event SPK OFF fail...\n");
				return -EINVAL;
			}
...
#if defined(CONFIG_SKY_EF51S_BOARD) || defined(CONFIG_SKY_EF51L_BOARD) 
...
#endif
#if defined(CONFIG_SKY_EF51S_BOARD) || defined(CONFIG_SKY_EF51L_BOARD) 
	{"AMIC1", NULL, "MIC BIAS5 External"},
	{"MIC BIAS5 External", NULL, "Handset Mic"},
#endif
};
...
//HDJ_LS4_Sound_20120503_END
#ifdef CONFIG_SKY_SND_AUTOANSWER
	SOC_ENUM_EXT("Autoans Flag", msm_enum[4], autoans_flag_get,
		autoans_flag_set),
#endif /* CONFIG_SKY_SND_AUTOANSWER */		
};
