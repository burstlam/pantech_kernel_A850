...
#if 1 //def FEATURE_PANTECH_SND_QCOM_CR //20120806 jhsong : qct patch
#define QCT_PATCH_SR_937289 1
#endif
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
#ifdef CONFIG_SKY_SND_FAB2210
	   if(is_headphone_up==0){  //20120811 jhsong : switch noise when seek next song or start sound
#endif
...
#if defined(CONFIG_SKY_EF51S_BOARD) || defined(CONFIG_SKY_EF51L_BOARD)
	SND_SOC_DAPM_AIF_OUT_E("SLIM TX7", "AIF2 Capture", 0, SND_SOC_NOPM, 7,
				0, tabla_codec_enable_slimtx,
				SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
#else
