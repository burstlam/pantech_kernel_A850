...
#define PANTECH_CHARGER_MONITOR_TEST
#if (CONFIG_BOARD_VER > CONFIG_TP20)
//#define USE_APQ_WAKEUP_MPP
#else
//#define USE_APQ_WAKEUP_GPIO
#endif
//#define PANTECH_USED_ALL_OTHER_WRITABLE_FILES
//#define DEBUG_TEMP_COMPENSATION_ALGO
//#define PANTECH_BATT_TEMP_COMPENSATION

#ifdef PANTECH_CHARGER_MONITOR_TEST
#include <linux/proc_fs.h>
#endif /* PANTECH_CHARGER_MONITOR_TEST */

#ifdef PANTECH_USED_ALL_OTHER_WRITABLE_FILES
#include <linux/uaccess.h>
#endif /* PANTECH_USED_ALL_OTHER_WRITABLE_FILES */

#ifdef CONFIG_PANTECH_BMS_TEST
...
void smb347_regs_init(void)
{
...
#if defined(CONFIG_MACH_APQ8064_EF50L)
	smb347_write_reg(0x00,0x9A);
#else
	smb347_write_reg(0x00,0x99);
#endif
	smb347_write_reg(0x01,0x17);
	smb347_write_reg(0x02,0x9F);
	smb347_write_reg(0x03,0x6A);
	smb347_write_reg(0x04,0xAE);
	smb347_write_reg(0x05,0x1E);
	smb347_write_reg(0x06,0x08);
	...
	smb347_write_reg(0x0D,0x10);
#else
	smb347_write_reg(0x30,0xC0);
	smb347_write_reg(0x31,0x03);
#if defined(CONFIG_MACH_APQ8064_EF50L)
...
void smb347_dump_regs(void)
{
	...
	for(i=0; i<16; i++)
		smb347_read_reg((0x00+i), &smb347_cmd_regs[i]);

	// dump status registers
	for(i=0; i<16; i++) 
		smb347_read_reg((0x30+i), &smb347_status_regs[i]);
...
#if 0 //def USE_SMB347_CABLE_DETECTION
...
static int smb347_subdevices_register(void)
{
	...
	if (!bms_input_attr_dev) {
	    	pr_err("BMS: Unable to register platform_device_register_simple device\n");
		rc = -ENXIO;
	    	return rc;
	}
	...
	if (rc) {
	pr_err("[BMS] failed: sysfs_create_group  [ERROR]\n");	
...
static int get_cable_type(struct smb347_chg_chip *chip, int level)
{
	...
	if(level == DRIVER_BOOT_COMPLETED) {
		/* APSD reset */
		smb347_write_reg(0x04,0xAA);
		msleep(1);
		smb347_write_reg(0x04,0xAE);
		
		/* check APSD Status */
		do {
			rc = smb347_read_reg(0x3E, &rdData);
			if(rc < 0) 
				break;
			msleep(10);
...
static void set_appropriate_battery_current(struct smb347_chg_chip *chip)
{
	...
	smb347_write_reg(0x30, 0xC0);
	smb347_write_reg(0x31, 0x03);
	smb347_write_reg(0x00, reg_ptr[chip->pantech_cable].charge_current);
	smb347_write_reg(0x01, reg_ptr[chip->pantech_cable].input_cur_limit);
	smb347_write_reg(0x03, reg_ptr[chip->pantech_cable].float_voltage);
	smb347_write_reg(0x30, reg_ptr[chip->pantech_cable].cmd_reg_a);
	smb347_write_reg(0x31, reg_ptr[chip->pantech_cable].cmd_reg_b);
...
#ifdef CONFIG_PANTECH_BMS_TEST
	if(chip->batt_soc <= 0 && (atomic_read(&bms_cutoff_flag)==0))
  		chip->batt_soc = 1;
...
static irqreturn_t smb347_chg_detection_handler(int irq, void *dev)
{
	...
	if(is_chg_plugged_in(chip)) {	
		...
		if(chip->pantech_cable == PANTECH_USB 
			|| chip->pantech_cable == PANTECH_AUDIO_DOCKING_STATION)
...
//		notify_usb_of_the_plugin_event(0);
		...
		smb347_write_reg(0x00, therm_normal_val[PANTECH_CABLE_NONE].charge_current);
		smb347_write_reg(0x01, therm_normal_val[PANTECH_CABLE_NONE].input_cur_limit);
		smb347_write_reg(0x03, therm_normal_val[PANTECH_CABLE_NONE].float_voltage);
		smb347_write_reg(0x30, therm_normal_val[PANTECH_CABLE_NONE].cmd_reg_a);
		smb347_write_reg(0x31, therm_normal_val[PANTECH_CABLE_NONE].cmd_reg_b);
...
static void chg_recheck_worker(struct work_struct *work)
{
	...
	if(is_chg_plugged_in(chip)) {
		...
		if(chip->pantech_cable != curr_cable_type) {
			...
			if(chip->pantech_cable == PANTECH_AUDIO_DOCKING_STATION) {
				if(curr_cable_type != PANTECH_AUDIO_DOCKING_STATION)
					switch_set_state(docking_speaker_sdev, 0);
				else
					switch_set_state(docking_speaker_sdev, 1);
			}
...
#ifdef PANTECH_CHARGER_MONITOR_TEST
static int proc_debug_pm_chg_get_Vchg(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	...
	*eof = 1;  
	...
	return sprintf(page, "%d\n",the_chip->charge_output_voltage);
					...
					int count, int *eof, void *data)
{
    ...
    if(NULL == the_chip)
    {
        pr_err("[proc_debug_pm_chg_get_BattID] the_chip is NULL(Error) !!!\n");
        return 0;
    }
					...
					int count, int *eof, void *data)
{
    if(NULL == the_chip)
    {
        pr_err("[proc_debug_pm_chg_get_CableType] the_chip is NULL(Error) !!!\n");
        return 0;
    }
...
static int proc_debug_pm_chg_get_regs(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	u8 reg_addr;
	u8 rdData;
	
	if(NULL == the_chip || NULL == data)
    	{
        	pr_err("[proc_debug_pm_chg_get_regs] the_chip is NULL(Error) !!!\n");
        	return 0;
    	}
	...
	smb347_read_reg(reg_addr, &rdData);
	...
	*eof = 1;  
	...
	if(reg_addr >= 0x30)
		return sprintf(page, "%s\n", &smb347_status_regs[reg_addr-0x30]); 
	else
		return sprintf(page, "%s\n", &smb347_cmd_regs[reg_addr]); 
#endif
}
...
static void create_testmenu_entries(struct smb347_chg_chip *chip)
{
	...
	ent = create_proc_entry("VCHG", 0, pm8921_charger_dir);
	if (ent == NULL) 
		pr_err("Unable to create /proc/VCHG entry\n");
	
	ent->read_proc = proc_debug_pm_chg_get_Vchg;        

	ent = create_proc_entry("smb347_regs", 0, pm8921_charger_dir);
	if (ent == NULL) 
		pr_err("Unable to create /proc/smb347_regs entry\n");
	
	ent->read_proc = proc_debug_pm_chg_get_regs;      
                                  ...
                                  const struct i2c_device_id *id)
{
	...
	ret_rd = smb347_read_reg(0x3F, &data);

	if(ret_rd < 0)
	{
		pr_err("%s smb347 reg 0x3F read failed & retry ~~\n", __func__);
		mdelay(200);
		smb347_read_reg(0x3F, &data);
	}
	return 0;
}
...
//	pr_err("## SMB347 Charger porbe OK ##\n");
