	       ...
	       unsigned int cmd, unsigned long arg)
{
	...
	switch(led_offset)
	{
		...
		case PM8XXX_ID_LED_2 :
			...
			pm_led_data[arg].cdev.brightness = (u8)cmd;
...
static void leds_diming_timer_check(struct pm8xxx_led_data *led)
{
	dbg("leds_diming_timer_check : led->cdev.brightness = %d \n", led->cdev.brightness);	
	if ( (led->cdev.brightness & 0xF0) > 0 ) {
		diming_end_value[led->id-PM8XXX_ID_LED_0] = led->cdev.brightness & ~PM8XXX_DRV_LED_CTRL_MASK;				
		diming_set_state[led->id-PM8XXX_ID_LED_0] = DIMMING_MODE_READY; //p12911
		diming_timer_count = 0;
		diming_oper_mode = DIMMING_STATE_LIGHTUP;
		diming_duration_time = DIMING_DATA_BASE_TIME +
			((led->cdev.brightness & 0xF0) >> PM8XXX_DRV_LED_CTRL_SHIFT)*DIMING_DATA_RESOLUTION;
	}
	else
	{
