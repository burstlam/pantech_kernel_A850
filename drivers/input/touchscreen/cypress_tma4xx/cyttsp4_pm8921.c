...
int hw_on_off(struct device *dev, bool on)
{
    struct regulator *vreg_touch_3_3, *vreg_touch_1_8;
    int rc =0;
    static bool vreg_touch_1_8_state = false;
    static bool vreg_touch_3_3_state = false;
    ...
    if(on != vreg_touch_1_8_state) {
        ...
        if(on) {
            ...
            if (rc) { 
                pr_err("set_voltage 8921_l21_1p8 failed, rc=%d\n", rc);
                return -EINVAL;
            }
            ...
            if (rc) { 
                pr_err("regulator_disable vreg_touch_1_8 failed, rc=%d\n", rc);
                return -EINVAL;
            }
        }
    ...
    if(on != vreg_touch_3_3_state) {
        ...
        if(on) {
            ...
            if (rc) { 
                pr_err("set_voltage 8921_l17_2p8 failed, rc=%d\n", rc);
                return -EINVAL;
            }
            ...
            if (rc) { 
                pr_err("regulator_disable vreg_touch_3_3 failed, rc=%d\n", rc);
                return -EINVAL;
            }
        }
