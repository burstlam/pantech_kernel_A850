...
#define MAX_LOGGER_BUF_SIZE 	255
static unsigned char klog_buf[MAX_LOGGER_BUF_SIZE+1];
extern void ram_android_logcat_write(const char *s, unsigned int count);
#endif
/*
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
	if (count < MAX_LOGGER_BUF_SIZE){		
		memcpy(klog_buf,log->buffer + log->w_off, count);	
		klog_buf[count] = 0x00;
	}
	else{
		memcpy(klog_buf,log->buffer + log->w_off, MAX_LOGGER_BUF_SIZE);	
		klog_buf[MAX_LOGGER_BUF_SIZE] = 0x00;
	}
#endif
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
{
	...
	if(klog_buf[0] > 0x00 && klog_buf[0] < 0x04)
	return ret;
	...
	index = strlen(klog_buf);	
	if(klog_buf[index-1] != 0x0A){  //last char != '\n'
		klog_buf[index] = 0x0A;
		index++;
	}	
	sprintf(TimeInfo,"[%02u:%02u:%02u.%03u]",hour,min,sec,msec);	

	ram_android_logcat_write(TimeInfo, strlen(TimeInfo));	
	ram_android_logcat_write(klog_buf, index);
}
#endif
...
#ifdef FEATURE_SKY_ADB_LOG_FOR_VITAMIN
