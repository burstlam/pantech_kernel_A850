...
static void VibeOSKernelLinuxStartTimer(void)
{
    ...
    else
    {
        ...
        */
        ...
        if (res != 0)
        {
            DbgOut("VibeOSKernelLinuxStartTimer: down_interruptible interrupted by a signal.\n");
        }
    }
    ...
    /* 
    ** For DRV2665, handling of NACKs while sending 40 bytes packet can take more than 5 ms.
    ** When this happens, the timer interrupt will release the g_hSemaphore while VibeOSKernelProcessData
    ** is executing and the player will immediately send a new packet to the SPI layer when VibeOSKernelProcessData
    ** exists. To avoid this, if the semaphore is released when VibeOSKernelProcessData exits,
    ** we acquire it to delay next packet sending to DRV2665 by 5 ms.
    */
