...
#error "SPI_HEADER_SIZE expected to be 3"
    ...
    {
        DbgOut("SaveOutputData: invalid write buffer size.\n");
        return 0;
    }
    ...
    {
        DbgOut("SaveOutputData: outputDataBuffer invalid.\n");
        return 0;
    }
    ...
    while (i < count)
    {
        ...
        if ((i + SPI_HEADER_SIZE) > count)
        {
            ...
            ** (Should never happen).
            */
            DbgOut("SaveOutputData: invalid buffer index.\n");
        }

        /* Check bit depth */
        if (8 != pInputBuffer->nBitDepth)
        {
            DbgOut("SaveOutputData: invalid bit depth. Use default value (8).\n");
        }
        ...
        if ((i + SPI_HEADER_SIZE + pInputBuffer->nBufferSize) > count)
        {
            ...
            ** (Should never happen).
            */
            DbgOut("SaveOutputData: invalid data size.\n");
        }
        
        /* Check actuator index */
        if (NUM_ACTUATORS <= pInputBuffer->nActuatorIndex)
        {
            DbgOut("SaveOutputData: invalid actuator index.\n");
...
bool SendOutputData(void)
{
    ...
    for (i = 0; i < NUM_ACTUATORS; i++) 
    {
        ...
        if (pCurrentActuatorSample->nBufferSize)
        {
            ...
            /* DbgOut("SendOutputData: 2: Size, BitDepth: %d, Buffer: %d, dataBuffer[0]:%d\n",
