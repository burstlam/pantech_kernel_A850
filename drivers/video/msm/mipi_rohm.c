 ...
 */
#ifdef CONFIG_F_SKYDISP_CABC_CTRL
static int cabc_control_skip_during_bootup =0;
void cabc_contol(struct msm_fb_data_type *mfd, int state)
{
	if(cabc_control_skip_during_bootup)
	{
	...
	cabc_control_skip_during_bootup = 1;
...
void SKY_LCD_CE_CASE_SET(struct msm_fb_data_type *mfd, uint32_t CEcase)
{
	...
	switch(CEcase) {
		case 0:  //default
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_off_cmds,
									ARRAY_SIZE(rohm_display_ce_off_cmds));
			break;
		case 1:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case1_set_cmds,
									ARRAY_SIZE(rohm_display_ce_case1_set_cmds));
			break;
		case 2:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case2_set_cmds,
									ARRAY_SIZE(rohm_display_ce_case2_set_cmds));
			break;
		case 3:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case3_set_cmds,
									ARRAY_SIZE(rohm_display_ce_case3_set_cmds));
			break;
		case 4:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case4_set_cmds,
									ARRAY_SIZE(rohm_display_ce_case4_set_cmds));
			break;
		case 5:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case5_set_cmds,
									ARRAY_SIZE(rohm_display_ce_case5_set_cmds));
			break;
		case 6:  
				mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_ce_case6_set_cmds,
...
{
	printk("[SKY_LCD] SKY_LCD_CE_CASE_GET : %d\n", ce_case_state);
...
static int mipi_rohm_lcd_off(struct platform_device *pdev)
{
	...
	if (rohm_state.disp_on == true) {
		...
		mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_black_data_cmds,	ARRAY_SIZE(rohm_display_black_data_cmds));
#endif
		mipi_dsi_cmds_tx(mfd, &rohm_tx_buf, rohm_display_off_cmds,	ARRAY_SIZE(rohm_display_off_cmds));
...
#ifdef CONFIG_F_SKYDISP_LCD_SHUTDOWN
extern void mipi_dsi_panel_power_shutdown(void);
void mipi_rohm_lcd_shutdown(void)
{
	struct msm_fb_data_type *mfd;
	struct fb_info *info; 
