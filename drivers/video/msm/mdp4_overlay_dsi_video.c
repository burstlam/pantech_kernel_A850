...
#include <linux/time.h>
#ifdef CONFIG_F_SKYDISP_QUALCOMM_SPC_VSYNC
...
#ifndef CONFIG_F_SKYDISP_QUALCOMM_ERROR_FIX
...
#ifdef CONFIG_F_ESCAPE_CODE_FOR_MDP4_WAITING
	int timeout;
#endif
...
#ifdef CONFIG_F_ESCAPE_CODE_FOR_MDP4_WAITING
	timeout = wait_for_completion_timeout(&dsi_video_comp, msecs_to_jiffies(VSYNC_PERIOD*2)); 
	if(timeout == 0) 
	{ 
		pr_err("#### %s: wait_for_completion_timeout .. \n", __func__); 
		mdp4_stat.err_vsync_timeout++; 
	} 
#else
 ...
 */
#ifdef CONFIG_F_SKYDISP_QUALCOMM_ERROR_FIX
...
static void mdp4_dsi_video_do_blt(struct msm_fb_data_type *mfd, int enable)
{
	...
	//patch 1032			enable, (int)dsi_pipe->blt_addr);
