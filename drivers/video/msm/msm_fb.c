...
/*20120523, kkcho, for offline charging image display */   
#ifndef CONFIG_PANTECH_OFFLINE_CHARGER_NOTIFY
#define CONFIG_PANTECH_OFFLINE_CHARGER_NOTIFY
#endif

#ifdef CONFIG_PANTECH_OFFLINE_CHARGER_NOTIFY
#undef CONFIG_PANTECH_CHARGER_OFFLINE
...
//#define BATT_CHARGING_IN "/hd_poweroff_icon_in.rle"
#endif
			...
			const char *buf,size_t count)
{
	...
	if(cabc_state == true)
		cabc_contol(mfd,true);
	else if(cabc_state == false)
		cabc_contol(mfd,false);
...
int ce_num;
extern void ce_contol(struct msm_fb_data_type *mfd, int count);
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	mfd->backlight_on_after_bootup =0;	
#endif
...
#elif defined(CONFIG_MACH_APQ8064_EF48S) ||defined(CONFIG_MACH_APQ8064_EF49K) || defined(CONFIG_MACH_APQ8064_EF50L) ||defined(CONFIG_MACH_APQ8064_EF51S) || defined(CONFIG_MACH_APQ8064_EF51L)
	/* 20120814, kkcho, Bug fix : display overlab at home-change
	* reproduce path : idle->power-key->display-off->power-key-> display overlab at display-on
	*/
	struct fb_info *fbi = mfd->fbi;
...
//#if defined(CONFIG_FB_PANTECH_MIPI_DSI_ROHM) // 20120829, kkcho, modify active-area for EF48/49/50 (116.736*65.664)
...
#elif defined(CONFIG_MACH_APQ8064_EF51S) || defined(CONFIG_MACH_APQ8064_EF51L)//p14682 kobj 120829 modify active-area EF51S/L(129.6*72.9)
	...
	printk(KERN_ERR "[SKY_LCD] fbram_size=%d >=  fix->smem_len=%d\n", fbram_size, fix->smem_len);//shkwak 
...
#else // #if defined(CONFIG_F_SKYDISP_BOOT_LOGO_IN_KERNEL) && defined(CONFIG_FB_MSM_LOGO)
#ifdef CONFIG_FB_MSM_LOGO
...
static int msmfb_overlay_play(struct fb_info *info, unsigned long *argp)
{
	...
	if (mfd->overlay_play_enable == 0)	/* nothing to do */
		return 0;
#ifdef CONFIG_F_PREVENT_RESET_DURING_BOOTUP
	if ((mfd->backlight_on_after_bootup  == 0)&& (info->node == 0))	/* nothing to do */
		return 0;
#endif
...
#ifndef CONFIG_F_PREVENT_RESET_DURING_BOOTUP
	if (info->node == 0 && !(mfd->cont_splash_done)) { /* primary */
		mdp_set_dma_pan_info(info, NULL, TRUE);
		if (msm_fb_blank_sub(FB_BLANK_UNBLANK, info, mfd->op_enable)) {
			pr_err("%s: can't turn on display!\n", __func__);
...
extern void SKY_LCD_CE_CASE_SET(struct msm_fb_data_type *mfd, uint32_t CEcase);
//extern uint32_t SKY_LCD_CE_CASE_GET(void);
#endif
...
#ifdef CONFIG_F_SKYDISP_CE_TUNING_M1
	case MSMFB_SKY_LCD_CE_CASE1:
		SKY_LCD_CE_CASE_SET(mfd, 1);
		//down(&msm_fb_ioctl_ppp_sem);
		//msm_fb_blank_sub_force(0, info, 1);
		//msm_fb_blank_sub_force(1, info, 1);
		//up(&msm_fb_ioctl_ppp_sem);
		break;
	case MSMFB_SKY_LCD_CE_CASE2:
		SKY_LCD_CE_CASE_SET(mfd, 2);
		break;
	case MSMFB_SKY_LCD_CE_CASE3:
		SKY_LCD_CE_CASE_SET(mfd, 3);
		break;
	case MSMFB_SKY_LCD_CE_CASE4:
		SKY_LCD_CE_CASE_SET(mfd, 4);
		break;		
	case MSMFB_SKY_LCD_CE_CASE5:
		SKY_LCD_CE_CASE_SET(mfd, 5);
		break;
	case MSMFB_SKY_LCD_CE_CASE6:
		SKY_LCD_CE_CASE_SET(mfd, 6);
		break;		
	case MSMFB_SKY_LCD_CE_CASE0:
		SKY_LCD_CE_CASE_SET(mfd, 0);
		break;		
#endif
