...
static void diag_read_hsic_work_fn(struct work_struct *work)
{
	 ...
	 */
	 pr_debug("* %d, %d\n",num_reads_submitted,err);
	// QC patch for kworker issue ->
	//if ((num_reads_submitted == 0) && (err != -ENODEV)){
	if ((driver->count_hsic_pool < driver->poolsize_hsic) && (num_reads_submitted == 0) && (err != -ENODEV)){
	// QC patch for kworker issue -<
...
static void diag_hsic_resume(void *ctxt)
{
	...
	if ((driver->count_hsic_pool < driver->poolsize_hsic) && ((driver->logging_mode == MEMORY_DEVICE_MODE) ||		
		(driver->usb_mdm_connected))){
	// QC patch for kworker issue -<
		queue_work(driver->diag_hsic_wq,
			 &driver->diag_read_hsic_work);
