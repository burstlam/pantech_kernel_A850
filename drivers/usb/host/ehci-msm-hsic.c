...
static int msm_hsic_suspend(struct msm_hsic_hcd *mehci)
{
	...
	/* 20120906 choiseulkee blk, Qualcomm Case #966766
	// QC Patch for PHY suspend issue
	if (!(readl_relaxed(USB_PORTSC) & PORT_PE)) {
		printk("^^^^^^ %s:port is not enabled skip suspend\n",
						__func__);
		return -EAGAIN;
	}*/
...
static int msm_hsic_resume(struct msm_hsic_hcd *mehci)
{
	...
	/* p10919 : 12/09/08 hello command fail test code */
	spin_lock_irqsave(&mehci->hsic_lock, flags);
	...
	/* p10919 : 12/09/08 hello command fail test code */
	spin_unlock_irqrestore(&mehci->hsic_lock, flags);
...
static int ehci_hsic_bus_suspend(struct usb_hcd *hcd)
{
	...
	//20120906 choiseulkee add, Qualcomm Case #966766
	if (!(readl_relaxed(USB_PORTSC) & PORT_PE)) {
		printk("%s:RH is not ready, skip suspend\n", 
			__func__);
		return -EAGAIN;
	}
...
static int __devinit ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	mehci->dev = &pdev->dev;
	pdata = mehci->dev->platform_data; /* 2012.09.05 EHCI: HCD: Set interrupt threshold value per controller basis */
