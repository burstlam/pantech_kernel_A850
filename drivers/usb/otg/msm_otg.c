...
static int msm_otg_reset(struct otg_transceiver *otg)
{
...
#if defined(FEATURE_HSUSB_SET_SIGNALING_PARAM)
	{
		...
		reg = ulpi_read(otg, 0x81);
		printk(KERN_ERR "HS DC Level pre-value:0x%x\n", reg);
		reg &= ~(0x0F);
		reg |= 0x0F; //24%
		ulpi_write(otg, reg, 0x81);
		reg = ulpi_read(otg, 0x81);
		printk(KERN_ERR "HS DC Level post-value:0x%x\n", reg);

		/*set the pre-emphasis value */		
		
		//there is a no ralation between pre-emphasis and USB Eye Diagram.
		/*
		reg = ulpi_read(otg, 0x82);
		printk(KERN_ERR "pre-emphasis pre-value:0x%x\n", reg);
		reg &= ~(3 << 4);
		reg |= (3 << 4); //reg |= (1 << 5); pre-emphasis  max
		ulpi_write(otg, reg, 0x82);
		reg = ulpi_read(otg, 0x82);
		printk(KERN_ERR "pre-emphasis post-value:0x%x\n", reg); 
		*/
		...
		reg &= ~(0x0F);
		printk(KERN_ERR "initial HS TX Rise/Fall time  value:0x%x\n", reg);
		//reg |= 0x07; //15%
		reg |= 0x0F; // -10%
		...
		reg =0;
			reg = ulpi_read(otg, 0x83);
//			printk("^^ Source impedance adjustment :0x%x\n", reg);
	}
	}
#endif
