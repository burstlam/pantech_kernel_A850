...
static struct msm_watchdog_pdata msm_watchdog_pdata = {
	.pet_time = 10000,
	//20120912 choiseulkee chg for 5 sec margin, Qualcomm Case #971263
	//.bark_time = 11000,
	.bark_time = 16000,
	.has_secure = true,
...
//#define DMOV_TSIF_CRCI         11 //APQ8064 TSIF port1 BSPdmovADM30.h
...
#define TSIF_1_CLK       GPIO_CFG(55/*59*/, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)
#define TSIF_1_EN        GPIO_CFG(56/*60*/, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)
#define TSIF_1_DATA      GPIO_CFG(57/*61*/, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)
...
struct msm_tsif_platform_data tsif1_platform_data = {
	...
	.init =  NULL//tsif_init1 [block wrong TSCR address access in MSM8960]
};
...
static struct fs_driver_data mdp_fs_data = {
	.clks = (struct fs_clk_data[]){
		...
		{ .name = "tv_src_clk" },
		{ .name = "tv_clk" },
#ifdef CONFIG_F_SKYDISP_QUALCOMM_FIX_MIPI_OPERATION_BECAUSE_MIPI_DOESNOT_WORK_AT_LOW_TEMP
		{ .name = "reset1_clk" },
		{ .name = "reset2_clk" },
#endif
		{ 0 }
...
void __init pantech_ram_console_debug_reserve(unsigned long ram_console_size)
{
	struct resource *res;
	long ret;

	pr_err("enter %s, %lu, 0x%x\n", __func__, ram_console_size, memblock_end_of_DRAM());
...
void __init pantech_ram_console_debug_init(void)
{
	int err;
