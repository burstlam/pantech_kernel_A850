...
// p16652 Add for enable/disable MDM DUMP MODE
...
/* for Ramconsole Backup */
static ssize_t rcsbackup_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	if(rcsbackup != 0 && rcsbackup != 1){
		return sprintf(buf, "error unknown value : %d\n", rcsbackup);
	} else
		return sprintf(buf, "%d\n", rcsbackup);
}

static ssize_t rcsbackup_store(struct kobject *kobj, struct kobj_attribute *attr,
							 const char *buf, size_t count)
{
	sscanf(buf, "%du", &rcsbackup);
	printk(KERN_INFO "%s : , rcsbackup : %d\n", __func__, rcsbackup);
		
	return count;
}

static struct kobj_attribute rcsbackup_attribute =
		__ATTR(rcsbackup, 0664, rcsbackup_show, rcsbackup_store);
#endif
// p16652 until here
